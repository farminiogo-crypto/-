...
- **الشيفت:** واحد مفتوح فقط في نفس الوقت. المتوقع في الدرج = `رصيد البداية + مبيعات كاش − مصروفات`. **الترابيزات المفتوحة مسموح تفضل مفتوحة بعد قفل الشيفت** — تُحمَل للشيفت الجاي والكاشير يكمّلها بكرة (المبيعات تتحسب على الشيفت اللي اتدفعت فيه). عند القفل يتطبع تقرير الشيفت (`ShiftReceipt`).
- **دفع جزئي (Split):** `POST /orders/:id/split-pay` بـ `items:[{item_id, qty}]` — بيعمل فاتورة جديدة مدفوعة بالأصناف المختارة (تطبع + تخصم مخزون + تتحسب مبيعات) ويقلّل كميات الترابيزة الأصلية؛ لو فضلت فاضية تتحرّر. المخزون بيتخصم عبر `deductInventoryForItems` (مشترك مع الدفع الكامل).
...
- **صوت + تنبيه الطلب الجديد:** في `POS.jsx` — التحديث الصامت بيقارن بصمة الترابيزات المشغولة (عدد + إجماليات)؛ لو زادت يبقى طلب نزل من الموبايل → نغمة `AudioContext` + توست. `prevSigRef` هو خط الأساس. التحديث بيتوقف لما `document.hidden` (تخفيف حِمل).
- **الكيبورد على الشاشة:** `components/OnScreenKeyboard.jsx` (مركّب في `Layout`). يظهر تلقائياً عند التركيز على أي خانة — حروف عربي/إنجليزي للنص، نمباد للأرقام (حسب `type`/`inputMode`). مفعّل فقط للشاشات ≥820px (ماكينة الكافيه)؛ الموبايل بيستخدم كيبورده الأصلي. بيكتب عبر `setNativeValue` عشان `onChange` بتاع React يشتغل. زر "تم" بيحترم `preventDefault` فما يعملش submit مزدوج.
- **أداء:** الأنيميشن اللا نهائي اتقلّل (شيلنا نبض أيقونة الترابيزة المشغولة) لتخفيف الحِمل المستمر على أجهزة ضعيفة (كان بيسبب تعليق مؤقت للّمس/الماوس).
